...
function buildPreCTA(cat: string, cat2: string | null, mixed: boolean): string {
  ...
  if (mixed && cat2) {
    ...
    if (COMBINATIONS[comboKey]) { obj = COMBINATIONS[comboKey].objetivos; res = COMBINATIONS[comboKey].resultados; }
    else { obj = PROFILE_DATA[cat]?.objetivos ?? []; res = PROFILE_DATA[cat]?.resultados ?? []; }
  } else { obj = PROFILE_DATA[cat]?.objetivos ?? []; res = PROFILE_DATA[cat]?.resultados ?? []; }
...
const QUESTIONS = [
  { q: (n: string) => `Cuando sales a caminar con ${n}...`, answers: [
  ...
  { q: (_n: string) => 'Cuando ve a otro perro o a una persona desconocida...', answers: [
  ...
  { q: (n: string) => `En casa, ${n} generalmente...`, answers: [
  ...
  { q: (n: string) => `Cuando le pides algo a ${n}`, qHighlight:'fuera de casa', qSuffix:'...', answers: [
  ...
  { q: (n: string) => `Con otros perros o personas desconocidas, ${n}...`, answers: [
  ...
  { q: (n: string) => `Cuando ${n} encuentra comida, un olor o algo interesante en el suelo...`, answers: [
...
function calcScore(answers: any[]) {
  const tot: Record<string,number> = {}; const cnt: Record<string,number> = {};
  TRAITS.forEach(t => { tot[t]=0; cnt[t]=0; });
  answers.forEach(a => { Object.entries(a.s).forEach(([t,v]) => { tot[t]+=v as number; cnt[t]++; }); });
  const avg: Record<string,number> = {};
  TRAITS.forEach(t => { avg[t] = cnt[t]>0 ? tot[t]/cnt[t] : 3; });
  const dists: Record<string,number> = {};
  Object.entries(VECTORS).forEach(([cat,vec]) => { dists[cat]=Math.sqrt(TRAITS.reduce((s,t)=>s+Math.pow(avg[t]-vec[t],2),0)); });
  const sorted = Object.entries(dists).sort((a,b)=>a[1]-b[1]);
  let primary=sorted[0][0], second=sorted[1][0]; const gap=sorted[1][1]-sorted[0][1];
  const ts=(answers[0]?.ts==='global'&&answers[4]?.ts==='global')?'global':(answers[0]?.ts==='social'||answers[4]?.ts==='social')?'social':'?';
  const pat=answers[3]?.pat==='con'?'con':answers[3]?.pat==='ctx'?'ctx':'?';
  if(gap<1.5){const pair=[primary,second].sort().join('|');
    if(pair==='dramatico|red_bull') primary=avg.sensibilidad>=4?'dramatico':'red_bull';
    if(pair==='alcalde_amiguero|red_bull') primary=avg.social<=1&&ts==='social'?'alcalde_amiguero':'red_bull';
    if(pair==='genio_selectivo|oportunista') primary=avg.conexion<=2&&pat==='con'?'oportunista':'genio_selectivo';
  }
...
function AvisoPrivacidad({dark=false,lead='Al continuar aceptas el tratamiento de tus datos conforme a nuestro'}:{dark?:boolean;lead?:string}) {
  const [open,setOpen]=useState(false);
  const soft = dark ? 'rgba(255,255,255,0.55)' : '#718096';
  const strong = dark ? 'rgba(255,255,255,0.9)' : NAVY;
  return <div style={{fontSize:11,lineHeight:1.6,color:soft,textAlign:'center'}}>
    {lead} <button type="button" onClick={()=>setOpen(!open)} style={{background:'none',border:'none',padding:0,font:'inherit',color:strong,textDecoration:'underline',cursor:'pointer'}}>aviso de privacidad</button>.
...
function ResultBody({c,c2,dogName,avg,showRadar,isMixed,restart,waUrl,preCTAText}:{c:any;c2?:any;dogName:string;avg:Record<string,number>;showRadar:boolean;isMixed:boolean;restart:()=>void;waUrl:string;preCTAText:string}) {
  ...
  return <>
    ...
    <div style={{padding:'16px 24px 24px',textAlign:'center'}}><button className="ghost-btn" onClick={restart}>Empezar de nuevo</button><div style={{marginTop:10}}><AvisoPrivacidad lead="Tus datos se tratan conforme a nuestro"/></div></div>
  </>;
}

const Q_LABELS = ['Caminata','Otro perro/persona','En casa','Fuera de casa','Social','Comida/olor'];
