...
function MiniRadar({ avg }: { avg: Record<string, number> }) {
  ...
  return (
    <svg viewBox="0 0 220 220" width="100%" style={{ maxWidth: 220, display: 'block', margin: '0 auto' }}>
      ...
      <polygon points={polyUser} fill="#0F2451" fillOpacity={0.35} stroke="#0F2451" strokeWidth={2} />
      {TRAITS.map((_, i) => {
        const a = (i / n) * 2 * Math.PI - Math.PI / 2;
        const lx = cx + (r + 24) * Math.cos(a);
        const ly = cy + (r + 24) * Math.sin(a);
        return (
          <text key={i} x={lx} y={ly} textAnchor="middle" dominantBaseline="central" fontSize={9} fill="#4A5568">
            {TRAIT_LABELS[i]}
          </text>
        );
      })}
