...
const TRAIT_LABELS = ["Activación","Control\nImpulsos","Sensibilidad","Control\nSocial","Conexión","Auto-ref."];
...
const COPY = {
  red_bull: {
    ...
    rec: n => `${n} vive a mil por hora: jala en la correa, ignora todo cuando se emociona y parece que nunca tiene apagador. No es que no te quiera escuchar — es que cuando está así, literalmente no puede.`,
    ...
    bridge: n => `Con un plan claro, ${n} puede aprender a regularse — y ese cambio transforma cada salida, cada visita y cada momento en casa.`,
  ...
  alcalde_amiguero: {
    ...
    rec: n => `${n} ama a todo el mundo — y ese amor no tiene filtro ni freno. Salta, jala, invade el espacio... y tú terminas disculpándote con todos en el parque.`,
    ...
    bridge: n => `Cuando ${n} entienda que la calma abre puertas — literalmente — todo cambia para él y para ti.`,
  ...
  protector_preocupado: {
    ...
    rec: n => `${n} ladra, se tensa o reacciona cuando algo le preocupa — y tú ya aprendiste qué situaciones lo van a disparar. Caminar con él a veces se siente como anticipar una crisis que no puedes evitar.`,
    ...
    bridge: n => `Este proceso requiere paciencia y un plan claro — pero cuando ${n} empiece a sentirse seguro, los cambios son profundos y duraderos.`,
  ...
  dramatico: {
    ...
    rec: n => `${n} puede estar tranquilo un momento y al siguiente estar en modo volcán — ladrando, jalando o haciendo drama total. No hay punto medio: o está bien o está completamente desbordado.`,
    ...
    bridge: n => `Con el entrenamiento correcto, ${n} puede aprender que no necesita explotar para conseguir lo que quiere.`,
  ...
  independiente: {
    ...
    rec: n => `Le hablas y no voltea. Lo llamas y sigue olfateando. En casa te hace caso — afuera es como si no existieras. ${n} tiene su propio mundo, y tú no siempre estás incluida.`,
    ...
    bridge: n => `Cuando ${n} descubra que orientarse hacia ti predice cosas buenas, todo lo demás se vuelve mucho más fácil.`,
    cta1: n => `El recall confiable de ${n} empieza aquí — y es más rápido de lo que crees.`,
  ...
  sombra: {
    ...
    rec: n => `${n} te sigue a todos lados, se angustia cuando no te ve y no puede quedarse solo sin que algo pase. Lo que muchos ven como amor total, tú sabes que a veces se siente agotador — para los dos.`,
    ...
    bridge: n => `Un perro con autonomía es más equilibrado, más feliz — y tú puedes moverte por tu casa sin escolta permanente.`,
  ...
  genio_selectivo: {
    ...
    rec: n => `En casa te hace caso perfecto. Afuera es como si nunca hubiera aprendido nada. ${n} sabe exactamente lo que le pides — solo que decide cuándo vale la pena hacerlo.`,
    ...
    bridge: n => `Cuando las reglas sean las mismas en todos lados, ${n} va a dejar de "elegir" — porque la respuesta correcta siempre va a tener valor.`,
  ...
  oportunista: {
    ...
    rec: n => `${n} roba comida, destruye cosas, se lanza hacia lo que le interesa — y lo hace con una eficiencia que casi da risa. Sabe exactamente lo que quiere y siempre encuentra la manera de conseguirlo.`,
    ...
    bridge: n => `Cuando tú seas la fuente más predecible de cosas buenas, ${n} va a preferir trabajar contigo — porque eso también tiene sentido para él.`,
...
function score(answers) {
  ...
  answers.forEach(a => {
    Object.entries(a.s).forEach(([t, v]) => { tot[t] += v; cnt[t]++; });
  });
  const avg = {};
  ...
  let primary = sorted[0][0], second = sorted[1][0];
...
function ProgressBar({ current, total }) {
  ...
  return (
    <div style={{ background: "#E2E8F0", borderRadius: 4, height: 4, width: "100%", overflow: "hidden" }}>
      <div style={{ background: GREEN, height: "100%", width: `${pct}%`, borderRadius: 4, transition: "width 0.3s ease" }} />
    </div>
  );
}
  ...
  return (
    <div className="app" style={{ background: "white", padding: "0 0 32px" }}>
      <div style={{ background: NAVY, padding: "40px 24px 32px", textAlign: "center" }}>
        <div style={{ fontSize: 13, color: GREEN, fontWeight: 700, letterSpacing: "0.1em", marginBottom: 8 }}>HABLA PERRO</div>
        <h1 style={{ color: "white", fontSize: 26, fontWeight: 700, lineHeight: 1.25, marginBottom: 12 }}>
          ¿Qué tipo de perro<br />es el tuyo?
        </h1>
        <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 15, lineHeight: 1.6, maxWidth: 320, margin: "0 auto" }}>
      ...
      <div style={{ padding: "32px 24px" }}>
        ...
        <div style={{ marginTop: 32, padding: "16px", background: WARM, borderRadius: 10 }}>
          <p style={{ fontSize: 12, color: DGREY, textAlign: "center", lineHeight: 1.5 }}>
            El único en México con certificación<br />reconocida mundialmente en entrenamiento canino.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="app" style={{ background: "white", padding: "0 0 32px" }}>
      <div style={{ background: NAVY, padding: "20px 24px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 16, minHeight: 28 }}>
          {qIndex > 0 && (
            <button className="btn-ghost" onClick={onBack} style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, padding: 0 }}>
              ← Anterior
            </button>
          )}
        </div>
      ...
      <div style={{ padding: "24px 24px 0" }}>
        <div style={{ fontSize: 17, fontWeight: 600, color: NAVY, lineHeight: 1.5, marginBottom: 20 }}>
          {q.qHighlight ? (
            <>
              {questionText}{" "}
              <span style={{ textDecoration: "underline", fontWeight: 700 }}>{q.qHighlight}</span>
              {q.qSuffix}
            </>
          ) : questionText}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {q.answers.map((a, i) => (
            ...
            >
              <span style={{ fontSize: 13, fontWeight: 700, color: selected === i ? "rgba(255,255,255,0.6)" : GREEN, marginRight: 8 }}>
                {["A","B","C","D"][i]}
              </span>
              {a.t}
            </button>
  ...
  return (
    <div className="app" style={{ background: NAVY, justifyContent: "center", alignItems: "center", padding: "40px 24px", minHeight: "100vh" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <h2 style={{ color: "white", fontSize: 22, fontWeight: 600, marginBottom: 8, lineHeight: 1.4 }}>
          Analizando el perfil<br />de {dogName}{".".repeat(dots + 1)}
        </h2>
        <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, lineHeight: 1.6, maxWidth: 280, margin: "0 auto 32px" }}>
          Cada perro tiene su propio patrón de comportamiento. Estamos encontrando el de {dogName}.
        </p>
        <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 4, height: 4, width: "100%", overflow: "hidden" }}>
          <div style={{ background: GREEN, height: "100%", width: `${progress}%`, borderRadius: 4, transition: "width 0.05s linear" }} />
        </div>
      </div>
    </div>
  );
...
function CustomRadarLabel({ x, y, cx, cy, index }) {
  ...
  return (
    <text x={x + dx} y={y + dy} textAnchor="middle" dominantBaseline="central" fontSize={11} fill={DGREY} fontFamily="sans-serif">
      {lines.map((line, i) => (
        <tspan key={i} x={x + dx} dy={i === 0 ? 0 : 13}>{line}</tspan>
      ))}
    </text>
  );
}

function RadarViz({ avg }) {
  const data = [
    { t: "Activación", v: +avg.activacion.toFixed(1) },
    { t: "Impulsos", v: +avg.impulsos.toFixed(1) },
    { t: "Sensibilidad", v: +avg.sensibilidad.toFixed(1) },
    { t: "Control Social", v: +avg.social.toFixed(1) },
    { t: "Conexión", v: +avg.conexion.toFixed(1) },
    { t: "Auto-ref.", v: +avg.auto_ref.toFixed(1) },
  ];
  return (
    <div style={{ width: "100%", height: 260 }}>
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart data={data} cx="50%" cy="50%" outerRadius={90}>
          ...
          <PolarAngleAxis dataKey="t" tick={{ fontSize: 11, fill: DGREY }} />
          <PolarRadiusAxis domain={[0, 5]} tick={false} axisLine={false} />
          <Radar dataKey="v" stroke={NAVY} fill={NAVY} fillOpacity={0.35} strokeWidth={2} />
        </RadarChart>
      </ResponsiveContainer>
    </div>
  );
    ...
    return (
      <div className="app" style={{ background: "white", paddingBottom: 48 }}>
        <div style={{ background: NAVY, padding: "32px 24px 28px" }}>
          <div style={{ display: "inline-block", background: "rgba(42,196,0,0.2)", border: "1px solid rgba(42,196,0,0.4)", borderRadius: 20, padding: "4px 14px", fontSize: 12, color: GREEN, fontWeight: 700, marginBottom: 12 }}>
            PERFIL COMBINADO
          </div>
          <h1 style={{ color: "white", fontSize: 24, fontWeight: 700, lineHeight: 1.3, marginBottom: 8 }}>
            {dogName} tiene un perfil combinado
          </h1>
          <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 15 }}>Eso no es ambigüedad — es precisión.</p>
        </div>
        <div style={{ padding: "24px" }}>
          <p style={{ color: DGREY, fontSize: 15, lineHeight: 1.7, marginBottom: 20 }}>
            Algunos perros no encajan perfectamente en una sola categoría. {dogName} muestra características de dos patrones distintos — lo que significa que su entrenamiento necesita atender más de una cosa al mismo tiempo.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
            {[c, c2].map((cat_copy, i) => (
              <div key={i} style={{ background: WARM, borderRadius: 10, padding: "16px 14px", textAlign: "center" }}>
                <div style={{ fontSize: 28, marginBottom: 6 }}>{cat_copy.emoji}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: NAVY, lineHeight: 1.3 }}>{cat_copy.name}</div>
                <div style={{ fontSize: 12, color: DGREY, marginTop: 4 }}>{cat_copy.energy}</div>
              </div>
            ))}
          </div>
          <div style={{ background: WARM, borderRadius: 10, padding: "20px 20px", marginBottom: 24 }}>
            <p style={{ color: DGREY, fontSize: 14, lineHeight: 1.6 }}>
              Un perfil combinado merece una conversación más que un resultado de pantalla.
            </p>
          </div>
          <div style={{ background: NAVY, borderRadius: 12, padding: "24px", marginBottom: 12 }}>
            <p style={{ color: GREEN, fontSize: 16, fontWeight: 700, marginBottom: 4 }}>¿Le cuento a Mike sobre {dogName}?</p>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, marginBottom: 16 }}>Este es exactamente el tipo de caso para el que estoy aquí.</p>
            <a href={WA_URL} target="_blank" rel="noreferrer" className="btn-wa">
              <span>💬</span> Escribirle a Mike
            </a>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 12, textAlign: "center", marginTop: 10 }}>Escribe CUERNAVACA · Sin compromiso</p>
          </div>
          <button onClick={onRestart} style={{ background: "none", border: "none", color: "#718096", fontSize: 13, cursor: "pointer", width: "100%", padding: "8px 0" }}>
            Empezar de nuevo
          </button>
        </div>
      </div>
    ...
    <div className="app" style={{ background: "white", paddingBottom: 48 }}>
      <div style={{ background: NAVY, padding: "32px 24px 28px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          ...
          <h1 style={{ color: "white", fontSize: 26, fontWeight: 700, lineHeight: 1.2 }}>{c.name}</h1>
        </div>
      ...
      <div style={{ background: WARM, padding: "24px" }}>
        ...
        <RadarViz avg={avg} />
        <p style={{ fontSize: 12, color: "#718096", textAlign: "center", marginTop: 4 }}>
          ⚠ Control de Impulsos y Control Social: puntaje bajo = menos regulación
        </p>
      </div>
      ...
      <div style={{ padding: "24px 24px 0" }}>
        ...
        <p style={{ color: DGREY, fontSize: 15, lineHeight: 1.7, marginBottom: 24 }}>
          {c.body}
        </p>
        ...
        <p style={{ color: DGREY, fontSize: 15, lineHeight: 1.7, marginTop: 12, marginBottom: 24 }}>
          {c.bridge(dogName)}
        </p>
      </div>

      <div style={{ background: NAVY, margin: "0 0 0", padding: "28px 24px" }}>
        <h3 style={{ color: "white", fontSize: 18, fontWeight: 700, marginBottom: 6, lineHeight: 1.4 }}>
          {c.cta1(dogName)}
        </h3>
        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, marginBottom: 16 }}>{c.sub1}</p>
        <p className="trust-line">El único en México con certificación reconocida mundialmente en entrenamiento canino</p>
        <a href={WA_URL} target="_blank" rel="noreferrer" className="btn-wa">
      ...
      <div style={{ padding: "24px", background: WARM }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
          <div style={{ padding: "14px", background: "white", borderRadius: 10, textAlign: "center" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: GREEN, letterSpacing: "0.06em", marginBottom: 4 }}>CERTIFICACIÓN</div>
            <div style={{ fontSize: 12, color: DGREY, lineHeight: 1.4 }}>Único en México con cert. reconocida mundialmente</div>
          </div>
          <div style={{ padding: "14px", background: "white", borderRadius: 10, textAlign: "center" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: GREEN, letterSpacing: "0.06em", marginBottom: 4 }}>CRUZ ROJA</div>
            <div style={{ fontSize: 12, color: DGREY, lineHeight: 1.4 }}>Entrenador oficial perros SAR · Cruz Roja Morelos</div>
          </div>
        </div>
        <div style={{ background: "white", borderRadius: 10, padding: "16px", marginBottom: 0 }}>
          <p style={{ fontSize: 14, color: DGREY, fontStyle: "italic", lineHeight: 1.6, marginBottom: 6 }}>
          ...
          <p style={{ fontSize: 12, color: "#718096" }}>— Cliente Habla Perro, Cuernavaca</p>
        </div>
      </div>

      <div style={{ padding: "28px 24px" }}>
        <h3 style={{ color: NAVY, fontSize: 17, fontWeight: 700, marginBottom: 6, lineHeight: 1.4 }}>
          {c.cta2}
        </h3>
        <p style={{ color: DGREY, fontSize: 13, marginBottom: 16 }}>Escribe CUERNAVACA y te cuento todo.</p>
        <a href={WA_URL} target="_blank" rel="noreferrer" className="btn-wa" style={{ background: NAVY }}>
          <span>💬</span> Hablar con Mike
        </a>
        <button onClick={onRestart} style={{ background: "none", border: "none", color: "#718096", fontSize: 13, cursor: "pointer", width: "100%", padding: "12px 0 0", display: "block", textAlign: "center" }}>
          Empezar de nuevo
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const restart = () => {
    ...
    setAnswers([]); setSelected(null); setProgress(0); setResult(null);
  };

  return (
    <>
      <style>{css}</style>
      {screen === "intro" && <IntroScreen dogName={dogName} setDogName={setDogName} onStart={() => dogName.trim() && setScreen("quiz")} />}
      {screen === "quiz" && <QuizScreen qIndex={qIndex} dogName={dogName} onAnswer={handleAnswer} onBack={handleBack} selected={selected} />}
      {screen === "loading" && <LoadingScreen dogName={dogName} progress={progress} />}
      {screen === "result" && result && <ResultScreen result={result} dogName={dogName} onRestart={restart} />}
    </>
  );
}
